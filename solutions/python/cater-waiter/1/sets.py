...
def categorize_dish(dish_name, dish_ingredients):
    ...
    # only if all ingredients are present in the category
    # so if a (lsit of ingredients) is subset of b
...
def tag_special_ingredients(dish):
    ...
    d, i = dish
    return (d, SPECIAL_INGREDIENTS.intersection(i))
...
def singleton_ingredients(dishes, intersection):
    ...
    # for each dish
    for dish in dishes:
        # find the elements that are in dish and NOT in intersection
        # because those in intersection appear twice and we need to
        # find the ones that appear once
        not_in_intersection = dish.difference(intersection)
        for singleton_i in not_in_intersection:
            singleton_is.append(singleton_i)
    return set(singleton_is)        
